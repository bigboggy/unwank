...
case "$(uname -s)" in
  ...
  Linux)
    ...
    ;;
  MINGW*|MSYS*|CYGWIN*)
    powershell.exe -NoProfile -Command "Start-Process -NoNewWindow powershell -ArgumentList '-NoProfile -Command (New-Object Media.SoundPlayer \"$RANDOM_FILE\").PlaySync()'" &>/dev/null &
    ;;
esac
